...
df$ID <- as.factor(seq.int(nrow(df)))
df$generator <- as.factor(df$generator)
df$terminal_streak_length <- as.factor(df$terminal_streak_length)
df$prediction_recode <- as.integer(df$prediction_recode)
sub_df <- df[c("ID", "generator", "terminal_streak_length", "prediction_recode")]
res <- anova_test(
  data = sub_df, dv = prediction_recode, wid = ID,
