...
avg_calc <- function(data, study){
  ...
  for(i in unique(x$participant_id)){
    ...
    df <- rbind(df, c(char, x[which(x$participant_id == i),]$generator[1], i, mean(x[which(x$participant_id == i),]$prediction_recode)))
  }
...
```{r}
permutation.test <- function(grp1, grp2, n, gener){
  grp1 <- avg_calc(grp1, 0)
  grp1 <- grp1[which(grp1$generator == gener),]
  a <- as.numeric(grp1$prediction_recode)
  grp2 <- avg_calc(grp2, 1)
...
# study 1
```{r}
test1 <- permutation.test(data1A, data1B, 10000, "analyst")
...
```
```{r}
test1 <- permutation.test(data1A, data1B, 10000, "bingo")
test2 <- unlist(test1[1])
non_signi <- unlist(test1[2])
...
```
```{r}
test1 <- permutation.test(data1A, data1B, 10000, "stock")
test2 <- unlist(test1[1])
non_signi <- unlist(test1[2])
